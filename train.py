...
@torch.no_grad()
def calculate_invalid_move_ratio(model, X, config, device):
    """
    Calculate the ratio of invalid moves generated by the model for a batch of data.
    
    Args:
        model: The model to evaluate
        X: Input tensor containing token sequences
        config (dict): Configuration dictionary
        device (str): Device to use for tensors
        
    Returns:
        float: Ratio of invalid moves (number of invalid moves / total moves)
    """
...
def estimate_loss(model, ctx, config, device, device_type):
    ...
    for split in ['train', 'val']:
        ...
        # For validation split with board game, also calculate invalid move ratio
        if split == 'val' and config.get('board_game'):
            invalid_move_ratios = torch.zeros(config['eval_iters'])
        else:
            invalid_move_ratios = None
        # end if
        ...
        # Add invalid move ratio to output for validation split with board game
        if split == 'val' and config.get('board_game'):
            out['invalid_move_ratio'] = invalid_move_ratios.mean()
        # end if
    # end for
    ...
    while True:
        ...
        if iter_num % config['eval_interval'] == 0 and master_process:
            ...
            if config['wandb_log']:
                ...
                if 'invalid_move_ratio' in losses:
                    log_data["val/invalid_move_ratio"] = losses['invalid_move_ratio']
                    print(f"Invalid move ratio: {losses['invalid_move_ratio']:.4f}")
                # end if
                wandb.log(log_data)
            # end if
